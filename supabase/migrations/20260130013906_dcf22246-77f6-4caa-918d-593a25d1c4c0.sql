-- Update the calculate_loan_eligibility function to use "Reverse" terminology
...
BEGIN
  ...
  -- ============================================================
  -- REVERSE ELIGIBILITY - RAK POS ONLY
  -- When eligible_multiplier = 0 (variance > 25%) and product is RAK POS
  -- Use REVERSE method: Loan = Adjusted Turnover, ABCT = 1% of Adjusted Turnover
  -- ============================================================
  IF NEW.product_type = 'rak_pos' 
     AND v_eligible_multiplier <= 0 
     AND v_adjusted_turnover > 0 THEN
    
    -- Use Reverse Method
    v_eligibility_method := 'Reverse';
    v_eligibility_status := 'Eligible (Reverse)';
    
    -- Loan = Adjusted Turnover (capped at Adjusted Turnover, no 8× multiplier)
    v_eligible_loan_amount := v_adjusted_turnover;
    
    -- ABCT = 1% of Adjusted Turnover
  ...
  -- Standard calculation path (NORMAL method)
  ...
  -- Calculate ABCT Fee (1% for RAK POS Loan only)
