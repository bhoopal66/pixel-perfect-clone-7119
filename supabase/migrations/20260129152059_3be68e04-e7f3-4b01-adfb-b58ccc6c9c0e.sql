-- Add eligibility method tracking column
ALTER TABLE loan_eligibility 
ADD COLUMN IF NOT EXISTS eligibility_method TEXT DEFAULT 'Standard';

-- Update the calculate_loan_eligibility trigger function to include alternative method for RAK POS
...
DECLARE
  ...
  v_eligibility_method TEXT := 'Standard';
BEGIN
  -- Treat nulls as 0
  ...
  IF NEW.declared_turnover = 0 OR NEW.declared_turnover IS NULL THEN
    ...
    NEW.eligibility_method := 'Standard';
    NEW.updated_at := now();
    RETURN NEW;
  END IF;
  ...
  -- ============================================================
  -- ALTERNATIVE ELIGIBILITY - RAK POS ONLY
  -- When variance > 25% and product is RAK POS, use ABCD reversal
  -- Loan = Adjusted Turnover, ABCD = 1% of Adjusted Turnover
  -- ============================================================
  IF NEW.product_type = 'rak_pos' 
     AND v_eligibility_status = 'Not Eligible' 
     AND v_adjusted_turnover > 0 THEN
    
    -- Use Alternative Method
    v_eligibility_method := 'Alternative';
    v_eligibility_status := 'Eligible (Alternative)';
    
    -- Loan = Adjusted Turnover
    v_eligible_loan_amount := v_adjusted_turnover;
    
    -- ABCD = 1% of Adjusted Turnover
    v_abcd_fee_rate := 0.01;
    v_abcd_fee_amount := ROUND((v_adjusted_turnover * 0.01)::NUMERIC, 2);
